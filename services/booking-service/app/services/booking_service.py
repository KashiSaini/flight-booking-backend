...
async def get_user_bookings(skip: int, limit: int, current_user: User, db: AsyncSession):
    result = await db.execute(
        select(Booking).where(Booking.user_id == current_user.id).offset(skip).limit(limit)
    )
...
async def cancel_booking(
    ...
    if booking.seat_id:
        seat_res = await db.execute(select(Seat).where(Seat.id == booking.seat_id).with_for_update())
        seat = seat_res.scalar_one_or_none()
        if seat:
            seat.is_booked = False
    elif booking.seat_number:
        seat_res = await db.execute(
            select(Seat).where(Seat.flight_id == booking.flight_id, Seat.seat_number == booking.seat_number).with_for_update()
        )
        seat = seat_res.scalar_one_or_none()
        if seat:
            seat.is_booked = False
