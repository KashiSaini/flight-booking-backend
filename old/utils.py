...
async def get_current_user(token: str, db):
    ...
    try:
        ...
        # Convert user_id to int since JWT stores it as string
        try:
            user_id = int(user_id)
...
async def log_user_activity(user_id: int, action: str, details: dict = None):
    log_entry = {
        "user_id": user_id,
        "action": action,
        "details": details or {},
